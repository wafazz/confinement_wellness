...
class BookingController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($request->filled('filter_state')) {
                $bookings->where('state', $request->filter_state);
            }
            if ($request->filled('filter_source')) {
                $bookings->where('source', $request->filter_source);
            }
            ...
            return DataTables::of($bookings)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if ($row->status === 'approved') {
                        $convertUrl = route('hq.bookings.convert-form', $row->id);
    ...
    public function convertForm(Booking $booking)
    {
        ...
        return view('hq.bookings.convert', compact('booking', 'leaders', 'therapists', 'serviceTypes'));
    }
    ...
    public function convert(Request $request, Booking $booking)
    {
        ...
        return redirect()->route('hq.bookings.index')->with('success', 'Booking converted to job ' . $jobCode . ' successfully.');
    }
}

...
@section('content')
<a href="{{ route('hq.bookings.show', $booking) }}" class="btn btn-sm btn-outline-secondary mb-3">
...
<div class="row g-4">
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                <form action="{{ route('hq.bookings.convert', $booking) }}" method="POST">
                    @csrf
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">Assign to Leader <span class="text-danger">*</span></label>
                            <select class="form-select @error('assigned_by') is-invalid @enderror" name="assigned_by" required>
                                <option value="">Select Leader</option>
                                @foreach($leaders as $leader)
                                    <option value="{{ $leader->id }}" {{ old('assigned_by') == $leader->id ? 'selected' : '' }}>
                                        {{ $leader->name }} ({{ $leader->state }})
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-select @error('assigned_to') is-invalid @enderror" name="assigned_to" required>
                                ...
                                @foreach($therapists as $therapist)
                                    <option value="{{ $therapist->id }}" {{ old('assigned_to', $booking->preferred_therapist_id) == $therapist->id ? 'selected' : '' }}>
                                        {{ $therapist->name }} ({{ $therapist->state }})
                                    </option>
                                @endforeach
                            </select>

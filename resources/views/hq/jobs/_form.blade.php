<div class="row">
    ...
    <div class="col-md-6 mb-3">
        <label for="assigned_by" class="form-label">Assigned By (Leader) <span class="text-danger">*</span></label>
        <select class="form-select @error('assigned_by') is-invalid @enderror" id="assigned_by" name="assigned_by" required>
            <option value="">Select Leader</option>
            @foreach($leaders as $leader)
                <option value="{{ $leader->id }}" {{ old('assigned_by', $job->assigned_by ?? '') == $leader->id ? 'selected' : '' }}>{{ $leader->name }} ({{ $leader->state }})</option>
            @endforeach
        </select>
        @error('assigned_by') <div class="invalid-feedback">{{ $message }}</div> @enderror
    </div>
    <div class="col-md-6 mb-3">
        <label for="assigned_to" class="form-label">Assigned To (Therapist) <span class="text-danger">*</span></label>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showDateRange(workDays) {
        ...
        // Copy value between single and start date

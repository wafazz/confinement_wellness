...
@section('content')
<a href="{{ route('leader.bookings.index') }}" class="btn btn-sm btn-outline-secondary mb-3">
...
<div class="row g-4">
    <div class="col-lg-8">
            ...
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                ...
                @php
                    $color = match($booking->status) {
                        'pending_review' => 'warning', 'approved' => 'info',
                        'converted' => 'success', 'rejected' => 'danger', default => 'secondary',
                    };
                @endphp
        ...
        <div class="card border-0 shadow-sm mt-3">
            <div class="card-header bg-white"><h6 class="mb-0"><i class="fas fa-briefcase me-2"></i>Linked Job</h6></div>
            <div class="card-body">
                ...
                @if($booking->serviceJob->assignee)
                    <p class="mb-0 small">Therapist: {{ $booking->serviceJob->assignee->name }}</p>
                @endif
                <a href="{{ route('leader.jobs.show', $booking->serviceJob->id) }}" class="btn btn-sm btn-outline-primary mt-2">View Job</a>
            </div>
        </div>
        @endif
    </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="card border-0 shadow-sm mb-3">
            ...
            <div class="card-body">
                <form action="{{ route('leader.bookings.approve', $booking) }}" method="POST" class="mb-3">
                    @csrf @method('PATCH')
                    <button type="submit" class="btn btn-success w-100" onclick="return confirm('Approve this booking?')">
                        <i class="fas fa-check me-1"></i>Approve
                    </button>
                </form>
                <form action="{{ route('leader.bookings.reject', $booking) }}" method="POST">
                    @csrf @method('PATCH')
                    <textarea class="form-control form-control-sm mb-2" name="admin_notes" rows="2" placeholder="Reason for rejection..." required></textarea>
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                <a href="{{ route('leader.bookings.convert-form', $booking) }}" class="btn btn-primary w-100">
